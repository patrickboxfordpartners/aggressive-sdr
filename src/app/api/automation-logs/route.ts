...
export async function GET(request: NextRequest) {
  ...
  if (logId) {
    const rows = await queryInternalDatabase(`SELECT al.*, ar.name as rule_name, ar.action_type as rule_action_type, ar.trigger_tags as rule_trigger_tags, ar.action_config as rule_action_config, ar.trigger_mode as rule_trigger_mode, ar.description as rule_description FROM automation_logs al LEFT JOIN automation_rules ar ON al.rule_id = ar.id WHERE al.id = $1`, [logId]);
...
export async function DELETE(request: NextRequest) {
  ...
  if (action === "bulk") {
    ...
    const rows = await queryInternalDatabase(`DELETE FROM automation_logs WHERE id IN (${placeholders}) RETURNING id`, ids);
  ...
  if (action === "clear") {
    ...
    const params: (string | number)[] = [];
    let idx = 1;
    if (ruleId) { conditions.push(`rule_id = $${idx++}`); params.push(ruleId); }
    if (status) { conditions.push(`status = $${idx++}`); params.push(status); }
    if (olderThanDays) { const days = parseInt(olderThanDays, 10); if (!isNaN(days) && days > 0) conditions.push(`created_at < NOW() - INTERVAL '${days} days'`); }
    if (conditions.length === 0) return NextResponse.json({ error: "At least one filter is required to clear logs" }, { status: 400 });
    const where = `WHERE ${conditions.join(" AND ")}`;
    const rows = await queryInternalDatabase(`DELETE FROM automation_logs ${where} RETURNING id`, params);
    return NextResponse.json({ deleted: rows.length });
  }
