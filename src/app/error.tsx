          ...
          <Collapsible open={isOpen} onOpenChange={setIsOpen} className="w-full mb-6 border rounded-lg overflow-hidden">
            ...
            <CollapsibleContent>
              <div className="p-4 bg-muted/50 space-y-2 text-sm font-mono overflow-auto max-h-[200px]">
                ...
                {error.stack && (
                  <div><span className="font-semibold">Stack:</span><pre className="mt-1 whitespace-pre-wrap break-words text-xs">{error.stack}</pre></div>
                )}
              </div>
              <div className="p-3 border-t flex justify-end">
                <Button variant="outline" size="sm" className="text-xs" onClick={copyToClipboard}>
                  {hasCopied ? (<><Check className="h-3 w-3 mr-1" />Copied</>) : (<><Copy className="h-3 w-3 mr-1" />Copy to clipboard</>)}
                </Button>
              </div>
            </CollapsibleContent>
          </Collapsible>
          <div className="flex gap-4 w-full justify-center">
            <Button variant="secondary" className="min-w-[120px]" onClick={() => window.location.reload()}>Reload</Button>
            <Button className="min-w-[120px]" onClick={() => reset()}>Try Again</Button>
          </div>
        </CardContent>
      </Card>
    </div>
